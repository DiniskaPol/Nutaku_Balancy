...
public class TestUIController : MonoBehaviour
{
    ...
    public Button GetMyProfileAndSubButton;
    public Button PostPaymentButton;
	public Button PostPlayFabPaymentButton;
	public Button crossDeeplinkButton;
    ...
    void Awake()
    {
        ...
        //RestApiHelper 3Legged: Get My Profile along with Nutaku Subscriber status
        GetMyProfileAndSubButton.onClick.AddListener(TestGetMyProfileAndSub);
        ...
        //RestApiHelper 3Legged: Send Make Request
        ...
        //Cross deeplink to game page of Nutaku Store if installed or Nutaku website
        crossDeeplinkButton.onClick.AddListener(TestCrossDeeplinkButton);
    }

    void TestCrossDeeplinkButton()
    {
        BeginLoading();
        try
        {
            logMessage("");
            logMessage("Crossing deeplink to fap-ceo");
            SdkPlugin.crossDeeplinkToGame("fap-ceo");
    ...
    void TestGetMyProfileAndSub()
    {
        BeginLoading();
        try
        {
            logMessage("");
            logMessage("GetMyProfileAndSub Start");
            RestApiHelper.GetMyProfileAndSub(this, TestGetMyProfileAndSubCallback);
        }
        catch (Exception ex)
        {
            logError("GetMyProfileAndSub Failure");
            DumpError(ex);
            EndLoading();
        }
    }

    void TestGetMyProfileAndSubCallback(RawResult rawResult)
    {
        try
        {
            if ((rawResult.statusCode > 199) && (rawResult.statusCode < 300))
            {
                var result = RestApi.HandleRequestCallback<Person>(rawResult);
                logMessage("GetMyProfileAndSub Success");
                logMessage("Http Status Code: " + result.statusCode);
                var profile = result.GetFirstEntry();
                OutputPersonData(profile, true);
                userIdText.text = profile.id;
                nicknameText.text = profile.nickname;
            }
            else
    ...
    void OutputPersonData(Person person, bool withSub = false)
    {
        ...
        if (withSub) logMessage("activeSub: " + (person.activeSub == "1" ? "Yes" : "No"));
    }
